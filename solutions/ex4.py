def train_torch_pruning(trial, obj, train_data, valid_data, epochs=10):
...
def cnn_naive_objective(trial, train_data, valid_data, pruning=True, epochs=10):
    ...
    if(pruning):
      return train_torch_pruning(trial, classifier, train_data, valid_data, epochs)
    else:
      return train_torch(trial, classifier, train_data, valid_data, epochs)

# ---------------- Running the study ------------- #
objective_pruning = partial(cnn_naive_objective, train_data=train_data, valid_data=test_data, pruning=True)
study_pruning = optuna.create_study(direction='maximize', pruner=optuna.pruners.SuccessiveHalvingPruner())
study_pruning.optimize(objective_pruning, timeout=timeout, show_progress_bar=True)  # Invoke optimization of the objective function.
print('Best score={:0.5f} obtained for parameters={} after {} trials.'.format(study_pruning.best_value, study_pruning.best_params, len(study_pruning.trials)))